...
public class ContactServiceImpl extends ServiceImpl<ContactMapper, WorkContactEntity> implements IContactService {
    ...
    public GetContactPageResponse getContactPage(GetContactRequest param, int empId, int corpId, ReqPerEnum perEnum) {
        ...
        List<ContactDataVo> voList = new ArrayList<>();
        for (WorkContactEmployeeEntity entity : entityList) {
    ...
    private List<ContactDataVo> pageContactList(List<ContactDataVo> list, Integer pageNum, Integer pageSize) {
    ...
    public GetContactInfoResponse getContactInfo(Integer contactId, Integer empId, Integer corpId) {
        ...
        List<GetContactRoomVo> contactRooms = contactRoomService.getBaseMapper().getContactRoomList(contactIds, null);
        //endregion
    ...
    public boolean insertAllContact(List<WorkContactEntity> contacts, Integer corpId) {
        ...
        Map<String, Integer> map = contactList.stream().collect(Collectors.toMap(WorkContactEntity::getWxExternalUserid, WorkContactEntity::getId));

        List<WorkContactEntity> updateList = new ArrayList<>();
        List<WorkContactEntity> insertList = new ArrayList<>();
        for (WorkContactEntity entity : contacts) {
            if (map.containsKey(entity.getWxExternalUserid())) {
                entity.setId(map.get(entity.getWxExternalUserid()));
    ...
    public void addExternalContactOfRoomAutoLogic(int corpId, int empId, int contactId, String contactName,
        ...
        if (roomAutoPullDetail != null) {
            ...
            for (int i = 0; i < roomJsonArray.size(); i++) {
                ...
                Long count = contactRoomService.lambdaQuery()
